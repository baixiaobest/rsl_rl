...
class PPO:
    ...
    def __init__(
        ...
        # Auxiliary lidar prediction head components (optional). Trains the actor's shared lidar encoder and
        # prediction head in a separate phase (own optimizer, iterations, and batch size).
        if lidar_prediction_cfg is not None and getattr(self._raw_actor, "enable_prediction_head", False):
            self.lidar_prediction = lidar_prediction_cfg
            self.pred_optimizer = resolve_optimizer(optimizer)(
                self._raw_actor.prediction_parameters(), lr=lidar_prediction_cfg["learning_rate"]
            )  # type: ignore
        else:
    ...
    def update(self) -> dict[str, float]:
        ...
        mean_pred_loss = 0 if self.lidar_prediction else None
        ...
        # Trains the actor's shared lidar encoder + prediction head on (obs, next-frame-target) pairs.
        if self.lidar_prediction:
            ...
            for obs_batch, target_batch in self.storage.prediction_mini_batch_generator(
                ...
                pred_loss.backward()
                nn.utils.clip_grad_norm_(self._raw_actor.prediction_parameters(), self.max_grad_norm)
    ...
    def save(self) -> dict:
        ...
        if self.pred_optimizer is not None:
            saved_dict["pred_optimizer_state_dict"] = self.pred_optimizer.state_dict()
        return saved_dict
    ...
    def load(self, loaded_dict: dict, load_cfg: dict | None, strict: bool) -> bool:
        ...
        if load_cfg.get("optimizer"):
            ...
            if self.pred_optimizer is not None and "pred_optimizer_state_dict" in loaded_dict:
                self.pred_optimizer.load_state_dict(loaded_dict["pred_optimizer_state_dict"])
